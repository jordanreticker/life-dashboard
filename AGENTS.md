...
3. **Supabase CLI setup.** Every `supabase` command needs `SUPABASE_ACCESS_TOKEN` loaded from `.env.local` (repo root, gitignored) plus `--linked --workdir $(pwd)` — otherwise it fails with 401. No MCP server is configured.
